...
public class AddUserRoleAction(BotServiceProvider botServices)
    : ActionBase(
        ACTION_ID,
        "Add User Discord Role",
    ...
    )
{
    ...
    {
        var logger = BotServices.GetLogger<AddUserRoleAction>();
        ...
        try
        {
            ...
            logger?.LogDebug("Attempting to add user {userId} to role {roleId} in guild {guildId}",
                userId,
                roleId,
                guildId);

            await discordClient.GetGuild(guildId).GetUser(userId).AddRoleAsync(roleId);
        }
        catch (Exception ex)
        {
            logger?.LogError(
